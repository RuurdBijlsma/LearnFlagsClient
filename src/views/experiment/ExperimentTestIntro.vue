<template>
    <div class="home" :style="{
        ...
        <v-card elevation="2" class="left-card mb-3"
            ...
            <v-card-title>Test phase</v-card-title>
            <div class="flags">
                <v-img :key="flag" v-for="flag in randomFlags" :aspect-ratio="3/2" :src="flag"/>
            </div>
            <v-card-text>
                <p>You will now start a test determining how well you've learned the set of flags.</p>
            </v-card-text>
            <v-card-actions>
                <v-btn rounded color="primary" :to="`/experiment/test/${subsetId}`">
                    <span class="ml-4 mr-4">Start test</span>
                </v-btn>
            </v-card-actions>
        </v-card>
    </div>
